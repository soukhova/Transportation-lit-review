...
<!--
NOTE: IS THE TEXT BELOW A LITTLE REDUNDANT AFTER MOVING THE CALLS TO THIS SECTION? AS: Agreed.
...
To conclude, transportation equity analysis is important. In this report, we outline a conceptual framework for structuring equity analysis that may move us closer to transport justice, summarize findings from 165 relevant transportation equity papers from the academic literature, and critically appraised these findings with the conceptual transport justice framework.

This framework can be succinctly summarized as a classification exercise that explicitly supports a "Why" rationale. In conducting an equity analysis, the subject of justice "Who" must contextualizes the process, "What" the distribution of transport-related benefits and burdens looks like, "How" these inequities are measured and benchmarked, surrounded by the "Where" across temporally changing "When"s all should be explicitly stated. In this way, the rationale for "Why" the equity analysis is important and what root cause of/for (in)justice is motivating the analysis in the first place may be creitical to ultimately mobilizing equity analyses into transport justice.

From this framework and the critical apraisal of the literature, we outline five call to actions for researchers and decision-makers that should be adopted to move equity analysis closer to transport justice. These include:
...
- More direct links between equity standards and experienced conditions and outcomes. 
- We shouldn't shy away from appraise of policies. Work towards analysis that can be internationally compared. 

As mentioned in the introductory paragraphs, defining _fairness_ is complex, but complexity is no excuse to shirk the task. A founding rationale for the root "Why?" of injustice is needed to inform equity analysis and set standards: why is it that Bezo (and the likes) ability to fly to space while others are firmly grounded on earth on penny-pinched public transport systems to Bezo-connected job opportunities unfair? What aspects? To whom and in what ways? The rationale for "Why" is fundamental to fairness and justice. We must be explicit, intentional and clear in our work in equity analysis such that standards can be created, equity analysis be tracked, and used to inform policy intervention that move towards justice.
